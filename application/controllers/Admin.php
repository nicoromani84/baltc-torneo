...
public function enviarNotificacion() {
	...
	public function cronRecordatorios() {
		...
		$query = $this->db
			->select('p.id, p.categoria, p.ronda, p.fecha, p.resultado, p.jugador1_id, p.jugador2_id, p.deadline')
			->from('partidos p')
			->where('p.deadline', $fecha_limite)
			->where('p.fecha IS NULL', null, false)
			->where('p.resultado IS NULL', null, false)
			->get();

		if($query->num_rows() == 0) {
			echo "Sin partidos pendientes para hoy";
			return;
		}
		...
		// Cargar email library
		$this->load->library('MY_Email');

		foreach($partidos as $p) {
			// Obtener nombres de jugadores
			$j1_query = $this->db->select('rp.partner_id')->from('reservations_partners rp')
				->where('rp.reservation_id', $p->jugador1_id)->get();
			$j1_partners = $j1_query->result();
			$j1_names = implode(' - ', array_map(function($x) {
				$partner = $this->db->where('id', $x->partner_id)->get('partners')->row();
				return strtolower($partner->name ?? 'Unknown');
			}, $j1_partners));

			$j2_query = $this->db->select('rp.partner_id')->from('reservations_partners rp')
				->where('rp.reservation_id', $p->jugador2_id)->get();
			$j2_partners = $j2_query->result();
			$j2_names = implode(' - ', array_map(function($x) {
				$partner = $this->db->where('id', $x->partner_id)->get('partners')->row();
				return strtolower($partner->name ?? 'Unknown');
			}, $j2_partners));
			...
			// Enviar a J1
			foreach($j1_partners as $partner) {
				$partner_data = $this->db->where('id', $partner->partner_id)->get('partners')->row();
				if(!$partner_data || !filter_var($partner_data->email, FILTER_VALIDATE_EMAIL)) continue;
		...
		echo "Recordatorios enviados: $enviados correos el " . date('Y-m-d H:i:s');
	}
}
